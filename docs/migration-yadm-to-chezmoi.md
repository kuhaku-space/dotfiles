...
| ファイル | プレフィックスの意味 | 内容 |
| --- | --- | --- |
| `run_once_after_05-apt-packages.sh` | 初回1回だけ | apt パッケージの不足分を install |
...
- 実行順は **ファイル名のソート順**（プレフィックスを除いた `05` → `10` → `30`）。setup（`10`）が apt で入る zsh / git / sudo に依存するため、apt（`05`）を先に置いた（当初は `15-` で setup の後だったが、ワンライナー導入時に `05-` へ前倒しした）。
...
H05=$(sha256sum run_once_after_05-apt-packages.sh | awk '{print $1}')
...
# scriptState に書き込む
for pair in "$H05:05-apt-packages.sh" "$H10:10-setup.sh" "$H30:30-mise-install.sh"; do
  key=${pair%%:*}; name=${pair#*:}
...
移行時点で残していた作業はすべて完了した。記録として残す。

1. **ghq リポジトリの移行差分を push** — 完了（`a1104e1 Migrate dotfiles management from yadm to chezmoi`）。
2. **古い yadm の削除** — 完了（mise から `chezmoi` に置換済み、`~/.local/share/yadm` 削除済み）。
3. **GitHub リポジトリ名** — 完了。origin は `ssh://git@github.com/kuhaku-space/dotfiles.git` を指す。

リポジトリ内に残る `yadm` への参照は、README の「yadm からの移行記録」リンクと本ドキュメントのみ（いずれも履歴としての言及）。
